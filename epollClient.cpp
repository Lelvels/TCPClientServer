...
int main(int argc, char *argv[]){
    ...
    do{
        ...
        event_count = epoll_wait(epfd, evlist, MAX_EVENTS, 30000);
        if(event_count == -1){
            if(errno == EINTR)
                continue;
            else
        ...
        for(int i=0; i<event_count; i++){
            printf("Reading file descriptor '%d' -- ", evlist[i].data.fd);
            if(evlist[i].data.fd == 0){
                bytes_read = read(evlist[i].data.fd, buffer, RECVBUFSIZE);
